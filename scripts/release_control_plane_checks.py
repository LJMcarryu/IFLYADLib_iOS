...
def selector_pattern(selector: str) -> re.Pattern[str]:
    require(re.fullmatch(r"[A-Za-z_]\w*(?::[A-Za-z_]\w*)*: ?", selector + " ") is not None,
            f"Objective-C selector 非法: {selector!r}")
    if ":" not in selector:
        return re.compile(
            rf"(?m)^[ \t]*[-+][ \t]*\([^\r\n)]*\)[ \t]*"
...
def validate_public_header_selectors(
    pod_root: str | Path,
    header_globs: Iterable[str],
    ...
    for pattern in patterns:
        ...
        require(not relative.is_absolute() and ".." not in relative.parts,
                f"公开头 glob 必须位于 POD_ROOT 内: {pattern}")
    ...
    for header in unique_headers:
        ...
        for selector in required_selectors:
            require(selector_pattern(selector).search(source) is not None,
                    f"{header} 缺少 Objective-C selector: {selector}")
...
def run_fixture(module_name: str, header_name: str, selector: str) -> None:
    require(re.fullmatch(r"[A-Za-z_]\w*", module_name) is not None,
            "fixture module name 非法")
    require(Path(header_name).name == header_name and header_name.endswith(".h"),
            "fixture header name 非法")
    with tempfile.TemporaryDirectory(prefix="control-plane-canary-") as directory:
        ...
        (actual_headers / header_name).write_text(
            fixture_declaration(selector), encoding="utf-8"
        )
        validated = validate_public_header_selectors(
            pod_root,
            [f"Headers/Public/{module_name}/{header_name}"],
            [selector],
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate":
            headers = validate_public_header_selectors(
                args.pod_root, args.header_glob, args.selector
            )
