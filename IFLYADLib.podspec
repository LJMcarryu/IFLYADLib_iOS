# IFLYADLib 6.0.2 模型 A 可组合分发（Core 必选 + 各格式可选）。
# 每个 vendored xcframework 只含自身 .o；对 Core/VideoUI 符号保留未定义引用，在 App link 时统一解析。
# s.source 指向本仓库 GitHub Releases tag 6.0.2 的合并 zip（含 7 个 xcframework device+simulator 切片
...
6.0.1 起支持按广告形式可组合接入（模型 A）：Core 必选 + Banner/Splash/Interstitial/NativeFeed/Reward 各格式按需选用，VideoUI 与资源由依赖图自动带入。
  ...
  s.source       = { :http => 'https://github.com/LJMcarryu/IFLYADLib_iOS/releases/download/6.0.2/IFLYADLib-modelA-6.0.2.zip' }
