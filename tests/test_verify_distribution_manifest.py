...
def make_formal_repository(root: Path) -> None:
    ...
    replacements = {
        "README.md": (
            ...
            ("> **6.2.3 准备边界**：版本清单已切换到精确 PENDING checksum 与 A/B 占位；正式签名资产、tag、Release 和匿名消费验证均未生成，当前 `6.2.3` URL 不可用于生产依赖。", f"> **发布边界**：`IFLYADLib-modelA-6.2.3.zip` 的冻结 SHA-256 为 `{combined_sha256}`；10 个资产已完成匿名下载复验。"),
            ("`releaseState=PENDING` 明确表示 `6.2.3` 不能发布；不得复用 `6.2.2` 的 checksum 或 A/B 提交。上文 `6.2.2` 的 tag、Release 与终验证据继续作为历史正式事实保留。", "`releaseState=FORMAL` 表示本版正式资产与发布元数据已冻结，公开发布另由不可变 tag、Release 与匿名下载终验证明。"),
            ("| 6.2.3 | 待发布 |", f"| 6.2.3 | {RELEASE_DATE} |"),
            ("- 当前 `Package.swift` 的 7 个 checksum 均为本版精确 PENDING 占位，尚未生成或核对；正式签名 zip 冻结后须整体回填，并在 Release 创建后与匿名下载件逐项复验。", "- `Package.swift` 的 7 个 checksum 已与冻结资产和匿名下载件逐项核对。"),
        ),
        "CHANGELOG.md": (
            ("## [6.2.3] - 待发布", f"## [6.2.3] - {RELEASE_DATE}"),
            ("- 正式签名资产、7 个 SwiftPM checksum、tag、Release 与匿名消费验证尚未生成；不得将本节写成已发布。", f"- 正式签名资产已冻结；`IFLYADLib-modelA-6.2.3.zip` 的冻结 SHA-256 为 `{combined_sha256}`，并已完成匿名下载复验。"),
        ),
        "RELEASING.md": (
            ("当前最新公开正式版仍是 [`6.2.2`](https://github.com/LJMcarryu/IFLYADLib_iOS/releases/tag/6.2.2)（2026-08-10）。`main` 正在准备 `6.2.3`；该版正式资产、checksum、tag、Release 与匿名消费验证均未生成。", f"当前最新公开正式版是 [`6.2.3`](https://github.com/LJMcarryu/IFLYADLib_iOS/releases/tag/6.2.3)（{RELEASE_DATE}）。`IFLYADLib-modelA-6.2.3.zip` 的冻结 SHA-256 为 `{combined_sha256}`，并已完成匿名下载复验。"),
            ("`releaseState=PENDING` 明确禁止候选、tag 或 Release 消费。正式产物冻结后必须整体回填 7 个 checksum 与两个不同的真实 A/B 提交；不得沿用 `6.2.2` 的值。", "`releaseState=FORMAL` 表示本版正式资产与发布元数据已冻结，公开发布另由不可变 tag、Release 与匿名下载终验证明。"),
        ),
        "SECURITY.md": (
...
class DistributionManifestTests(unittest.TestCase):
    ...
    def test_formal_repository_passes_tag_and_formal_modes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            copy_contract_files(root)
            make_formal_repository(root)

            self.assertEqual(verify(root, VERSION, "tag"), "正式发布复验")
            self.assertEqual(verify(root, VERSION, "formal"), "正式发布复验")

    def test_formal_repository_rejects_pending_wording(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            copy_contract_files(root)
            make_formal_repository(root)
            ...
            with self.assertRaisesRegex(AssertionError, "残留 PENDING"):
